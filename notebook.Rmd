...
The graph topology together with the edge lengths and admixture proportions define a covariance matrix $\Sigma$ where entry $\sigma_{ij}$ is given by the $F_2(i,j)$ statistics. This statistics we can both infer from the data and extract from a fitted graph. I haven't fitted the graph here, so the covariance matrix shown below is completely arbitrary, but it is extracted from the graph.

```{r, echo=FALSE}
# Here I'm setting the diagonal to be 1 rather arbitrary... should estimate it.
extract_covariance_matrix <- function(graph, env, diag = rep(1.0, length(graph$leaves))) {
  populations <- graph$leaves
  Sigma <- matrix(NA, nrow = length(populations), ncol = length(populations))
  for (i in seq_along(leaves)) {
    for (j in seq_along(leaves)) {
      if (i != j)
        Sigma[i, j] <- eval(sf2(graph, leaves[i], leaves[j]), env)
    }
  }
  diag(Sigma) <- diag
  rownames(Sigma) <- colnames(Sigma) <- graph$leaves
  Sigma
}

Sigma <- extract_covariance_matrix(graph, list2env(list(
  ...
  edge_v_xx = 0.0001, edge_xx_EU = 0.001, edge_Y_U = 0.01, edge_w_v = 0.003, 
...
```{r}
mu <- rep(0.5, length(graph$leaves))
names(mu) <- graph$leaves
...
cond_F4 <- conditional_expected_F4(model, "D", "V", "Af", "C", "EU")
plot(freqs, cond_F4(freqs), type = 'l', xlab = "EU allele freq", 
     ylab = quote(F[4](D, V, Af, C)))

cond_F4 <- conditional_expected_F4(model, "D", "V", "EA", "C", "Af")
plot(freqs, cond_F4(freqs), type = 'l', xlab = "Af allele freq", 
     ylab = quote(F[4](D, V, EA, C)))

cond_F4 <- conditional_expected_F4(model, "EU", "EA", "V", "C", "Af")
plot(freqs, cond_F4(freqs), type = 'l', xlab = "Af allele freq", 
     ylab = quote(F[4](EU, EA, V, C)))
```
